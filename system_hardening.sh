...
show_summary() {
    ...
    echo "  QEMU guest agent:      $([[ "${INSTALL_QEMU_GUEST_AGENT}" -eq 1 ]] && echo "install/enable" || echo "skip")"
...
run_cmd_quiet() {
    # Sends noisy command output to the logfile while keeping the console readable.
    log "RUN (quiet): $*"
    if [[ -n "${LOGFILE}" ]]; then
        "$@" >> "${LOGFILE}" 2>&1
    else
...
# Keep PAM account/session processing enabled on Debian/Ubuntu; keyboard-interactive is disabled separately.
UsePAM yes
